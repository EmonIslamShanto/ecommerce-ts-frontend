...
const Dashboard = () => {
  ...
  const { isLoading, data, isError } = useGetStatsQuery(user?._id!);

  const stats = data?.stats;

  const { last6Months} = lastMonths();
    ...
    <div className="admin-container">
      ...
      <main className="dashboard">
        ...
        <section className="widget-container">
          <WidgetItem
            percent={stats?.revenue.change ?? 0}
            amount={true}
          ...
          />
          <WidgetItem
            percent={stats?.users.change ?? 0}
            value={stats?.users.totalUsers ?? 0}
            color="rgb(0 198 202)"
            heading="Users"
          />
          <WidgetItem
            percent={stats?.orders.change ?? 0}
          ...
          <WidgetItem
            percent={stats?.products.change ?? 0}
